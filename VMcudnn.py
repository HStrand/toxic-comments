...
class LstmNet():
    ...
    def __init__(self, embed_size, max_features, maxlen, embedding_matrix, num_features):
        ...
        model1 = Bidirectional(CuDNNLSTM(300, return_sequences=True, dropout=0.1, recurrent_dropout=0.1))(model1)
    ...
    def submit(self):
        ...
        sub.to_csv('submissions\\lstm12.csv', index=False)
...
if __name__ == "__main__":
    ...
    if oof:
        ...
        sub_oof = pd.read_csv('submissions\\oof_traingru5.csv', encoding="utf-8")
        for i in range(0,len(list_classes)):
            sub_oof[list_classes[i]][pred_idx] = y_oof[:,i]
        sub_oof.to_csv('submissions\\oof_traingru5.csv', index=False, encoding="utf-8")
